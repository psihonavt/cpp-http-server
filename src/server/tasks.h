...
private:
    ...
    std::optional<TaskResult> m_result;
...
public:
...

    {
        ...
        if (m_pid == 0) {
            ...
            // close read-ends (the parent process will use these for reding; widowing the pipe ...)
            close(stdout_pipe[0]);
            close(stderr_pipe[0]);
    ...
    {
        return m_state == TaskState::FINISHED && m_result && m_result->exit_code == 0;
    }
    ...
    friend void safe_task_cb_call(Task const& t);
...
public:
    ...
    std::optional<int> get_next_task_expires_in();

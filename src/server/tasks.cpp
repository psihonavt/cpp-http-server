...
void safe_task_cb_call(Task const& t)
{
    if (t.m_result && t.m_on_finished_cb) {
        CPPTRACE_TRY
        {
            (*t.m_on_finished_cb)(*t.m_result);
        }
...
void Task::mark_as_completed(int exit_code, int exit_by_signal)
{
    ...
    m_result = TaskResult { .exit_code = exit_code,
        .exit_by_signal = exit_by_signal,
        .stdout_content = read_fd(m_stdout),
        .stderr_content = read_fd(m_stderr) };
    ...
    safe_task_cb_call(*this);
}

void Task::mark_as_expired()
{

    m_result = TaskResult { .exit_code = -1,
        .exit_by_signal = -1,
        .stdout_content = read_fd(m_stdout),
        .stderr_content = read_fd(m_stderr) };
...
std::optional<int> Queue::get_next_task_expires_in()
{
    ...
    if (!expiry_tstamps.empty()) {
        ...
        return static_cast<int>(std::chrono::duration_cast<std::chrono::milliseconds>(*closest_et - now).count());
    }
    return std::nullopt;
}
...
void Queue::process_tasks()
{
    ...
    auto next_task_expires_in = get_next_task_expires_in();
    if (finished || expired || next_task_expires_in) {
        if (next_task_expires_in && m_arm_timer_cb) {
            m_arm_timer_cb(*next_task_expires_in + 1);
        }
